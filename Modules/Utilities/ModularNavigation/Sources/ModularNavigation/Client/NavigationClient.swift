...
public struct NavigationClient<Destination: Hashable> {
    ...
    init(
        ...
        self.present = { destination, mode in
            switch mode {
            case .root:
                // Root mode doesn't make sense for presentation
                assertionFailure("Cannot present with .root mode")
            case .push:
                coordinator.push(destination)
            case .sheet(let detents):
                coordinator.presentSheet(destination, detents: detents)
            case .cover:
                coordinator.presentFullScreenCover(destination)
            }
        }
        self.dismiss = {
            coordinator.dismiss()
...
extension NavigationClient {
    ...
    /// Present as a sheet
    public func presentSheet(_ destination: Destination, detents: Set<PresentationDetent> = [.large]) {
