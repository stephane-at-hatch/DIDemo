...
final class NavigationCoordinator<Destination: Hashable>: Hashable {
    ...
    func push(_ destination: Destination) {
        append(
            NavigationStep(destination: destination, mode: .push)
        )
    }
    ...
    /// Present a destination as a sheet with configurable detents.
    /// Has no effect if a sheet or cover is already presented.
    func presentSheet(_ destination: Destination, detents: Set<PresentationDetent> = [.large]) {
        append(
            NavigationStep(destination: destination, mode: .sheet(detents: detents))
        )
    }
    ...
    func presentFullScreenCover(_ destination: Destination) {
        append(
            NavigationStep(destination: destination, mode: .cover)
        )
    }
    ...
    private func append(_ step: NavigationStep<Destination>) {
        ...
        case .sheet(let presentationDetents):
            ...
            presentationBindings?.wrappedValue.sheet = IdentifiableBox(value: (step.destination, presentationDetents))
        case .cover:
            // Don't present if we already have a cover or sheet
            guard presentationBindings?.wrappedValue.sheet?.value == nil else { return }
            guard presentationBindings?.wrappedValue.fullScreenCover?.value == nil else { return }
            presentationBindings?.wrappedValue.fullScreenCover = IdentifiableBox(value: step.destination)
        }
    }
...
struct PresentationBindings<Destination> {
    ...
    var sheet: IdentifiableBox<(destination: Destination, detents: Set<PresentationDetent>)>?
    /// Current full screen cover presentation
    var fullScreenCover: IdentifiableBox<Destination>?
}
